...
**Four verdict sources** (coverage / lastTestedStamp):

1. **Live tip** — `GET /api/version` `releaseSha` (near-real-time via `refresh-dashboard.mjs`)
2. **Merged PRs** — `gh pr list` on `HelgoIQ-Organization/HelgoIQ-Platform`
3. **#1180 + hub drops** — running-list issue comments plus `drops/*/SUMMARY.md` and `index.json`
4. **Claude Code PR review comments** — `gh api` pull-review comments, issue comments, and review bodies on those recently merged PRs (Claude posts on Platform PRs generally, not only #1180). Matched by author login (`claude…`) or body markers (`Claude Code`, `generated with Claude`, etc.). No invented tokens; skipped cleanly if `gh` fails.

Also consulted when present: `MASTER.csv`, Cluster A FINAL.
...
`source.sources` lists the four verdict inputs above. `claudeReviews[]` holds matched Claude Code / automated review comments (PR number, excerpt, verdict, merge SHA, comment URL). Empty when `gh` cannot see HelgoIQ-Platform — that is expected, not invented data.
