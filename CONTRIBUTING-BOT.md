...
## Declan + Claude dual reports (standing)

For any progress / money / walk report Declan will read:

1. Ship **two files** in the drop: `FOR-DECLAN.md` (plain English, no tip hashes / permission codes / lane jargon) and `FOR-CLAUDE.md` (technical FAIL-first).
2. Title and `--plain=` must use ordinary searchable words (e.g. "Full money walk report"), never only "FAIL-first" / "rollup" / "matrix L#".
3. When telling Declan in chat: **never** rely on bare filename pills (`FOR-DECLAN.md`) — they do not open. Always attach the file(s) in chat **and** paste clickable https links, e.g.
   - Pages: `https://helgoiq-organization.github.io/helgoiq-work-hub/drops/<id>/FOR-DECLAN.md`
   - GitHub: `https://github.com/HelgoIQ-Organization/helgoiq-work-hub/blob/main/drops/<id>/FOR-DECLAN.md`
