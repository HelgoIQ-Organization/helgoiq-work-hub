(function () {
  ...
  const VIEWS = ["dashboard", "coverage", "feel", "tracker", "admin-nav", "drops"];
  const COVERAGE_SUBS = {
  ...
  const PLATFORM_STATUS_LABELS = Object.fromEntries(PLATFORM_STATUSES.map((s) => [s, s]));
  const BUILDER_FILTER_ORDER = ["codex", "claude", "cursor", "grok", "declan", "external", "unassigned"];
  ...
  function showView(name) {
    ...
    if (next === "admin-nav" && window.HelgoAdminNav && typeof window.HelgoAdminNav.ensure === "function") {
      window.HelgoAdminNav.ensure();
    }
  }
  ...
  function displayBuilder(builder) {
    return BUILDER_LABELS[builder] || (builder ? builder : "No agent");
  }

  function matchesTracker(item) {
    const f = state.trackerFilters;
    if (f.platformStatus.size && !f.platformStatus.has(item.platformStatus)) return false;
    if (f.state.size && !f.state.has(item.state)) return false;
    if (f.area.size && !f.area.has(item.area)) return false;
    if (f.builder.size && !f.builder.has(item.builder || "unassigned")) return false;
  ...
  function groupKey(item) {
    ...
    if (g === "builder") return displayBuilder(item.builder);
  ...
  function groupOrder() {
    ...
    if (state.trackerGroup === "builder") return BUILDER_FILTER_ORDER.map(displayBuilder);
    return [];
  }
  ...
  function renderTrackerChips() {
    ...
    const presentStatuses = unique("platformStatus");
    const statusOrder = [
      ...PLATFORM_STATUSES,
      ...presentStatuses.filter((status) => !PLATFORM_STATUSES.includes(status)),
    ];
    const presentBuilders = unique("builder");
    const builderOrder = [
      ...BUILDER_FILTER_ORDER,
      ...presentBuilders.filter((builder) => !BUILDER_FILTER_ORDER.includes(builder)),
    ];
    fill("trackerPlatformStatusChips", statusOrder, "platformStatus", PLATFORM_STATUS_LABELS);
    fill("trackerStateChips", unique("state"), "state");
    fill("trackerAreaChips", unique("area"), "area");
    fill("trackerBuilderChips", builderOrder, "builder", BUILDER_LABELS);
    fill(
      "trackerAgeChips",
  ...
  function renderTracker() {
    ...
    const privateCount = (data && Number(data.privateItemCount)) || 0;
    const privateByStatus = (data && data.privateByPlatformStatus) || null;
    const live = all.filter((item) => !item.seed);
    const list = all.filter(matchesTracker);
    const byStatus = {};
    if (privateByStatus && typeof privateByStatus === "object") {
      Object.keys(privateByStatus).forEach((status) => {
        byStatus[status] = privateByStatus[status] || 0;
      });
    } else {
      live.forEach((item) => {
        byStatus[item.platformStatus] = (byStatus[item.platformStatus] || 0) + 1;
      });
    }
    const statusDenom = privateCount || live.length;
    const doneish = (byStatus["Fixed — awaiting confirmation"] || 0) + (byStatus["Verified by agent"] || 0);
    const pct = statusDenom ? Math.round((doneish / statusDenom) * 100) : 0;
    const ring = $("trackerRing");
    const ringValue = $("trackerRingValue");
    const ringLabel = $("trackerRingLabel");
    if (ring) {
      ring.style.setProperty("--tracker-progress", `${pct}%`);
      ring.setAttribute(
        "aria-label",
        privateCount
          ? `${privateCount} private items hidden from public Hub; ${pct}% fixed or verified among hidden counts`
          : `${pct}% fixed or verified: ${doneish} of ${live.length} live items`
      );
    }
    if (ringValue) ringValue.textContent = privateCount ? String(privateCount) : `${pct}%`;
    if (ringLabel) {
      ringLabel.textContent = privateCount
        ? `${privateCount} private items (titles hidden) · ${doneish} fixed/verified in counts`
        : `${doneish} of ${live.length} live items (Fixed or Verified)`;
    }

    const statusBars = $("trackerStatusBars");
    if (statusBars) {
      const statusOrder = [
        ...PLATFORM_STATUSES,
        ...Object.keys(byStatus).filter((status) => !PLATFORM_STATUSES.includes(status)),
      ];
      statusBars.innerHTML = statusOrder
        .map((status) => {
          const n = byStatus[status] || 0;
          const statusPct = statusDenom ? Math.round((n / statusDenom) * 100) : 0;
          return (
            '<div class="tracker-status-row"><div class="tracker-status-label"><span>' +
            escapeHtml(status) +
            '</span><strong>' +
            n +
            " (" +
            statusPct +
            '%)</strong></div><div class="tracker-status-track"><span style="width:' +
            statusPct +
            '%"></span></div></div>'
          );
        })
    ...
    const blocking = live.filter((item) => item.launchBlocking).length;
    const launchBadge = $("trackerLaunchBadge");
    if (launchBadge) {
      launchBadge.textContent = `${blocking} launch-blocking · Dashboard readiness unchanged`;
      launchBadge.setAttribute("aria-label", `${blocking} launch-blocking Platform items; Dashboard launch readiness unchanged`);
    }
    if (stats) {
      const values = privateCount
        ? [
            [privateCount, "private items (hidden)"],
            [doneish, "fixed / verified (counts)"],
            [all.length, "public cards shown"],
            [0, "titles on public Hub"],
          ]
        : [
            [live.length, "live items"],
            [doneish, "fixed / verified"],
            [blocking, "launch blockers"],
            [all.filter((item) => item.seed).length, "seed examples"],
          ];
    ...
    board.innerHTML = "";
    if (!list.length) {
      board.innerHTML = privateCount
        ? '<div class="tracker-empty"><strong>' +
          privateCount +
          " private items</strong> are hidden from this public Hub (security, isolation, personal data, payment faults, commercial). Counts above only — no titles or evidence. Full board returns after Declan approves a private Hub design.</div>"
        : '<div class="tracker-empty">No tracker rows match these filters.</div>';
      return;
    }
    keys.forEach((key) => {
      ...
      items
        ...
        .forEach((item) => {
          ...
          const agent = displayBuilder(item.builder);
          row.innerHTML =
            "<div><p class='title'>" +
            ...
            (item.platformStatus
              ? '<span class="trk-pill platform-status">' +
                escapeHtml(item.platformStatus) +
                "</span>"
              : "") +
            ...
            escapeHtml(item.state || "") +
            "</span>" +
            '<span class="trk-pill agent-pill">' +
            escapeHtml(agent) +
            "</span>" +
            '<span class="trk-pill">' +
            escapeHtml(item.area || "") +
            "</span>" +
            (item.launchBlocking ? '<span class="trk-pill launch-blocking">launch-blocking</span>' : "") +
            '<span class="trk-pill">' +
            days +
            "d</span>" +
          ...
          if (!evidence.length && item.agentNotes) {
            const p = document.createElement("p");
            p.className = "muted";
            p.textContent = item.agentNotes;
            ev.appendChild(p);
          } else if (!evidence.length) {
            const p = document.createElement("p");
            p.className = "muted";
            p.textContent = item.seed ? "No evidence links on this seed row." : "No evidence links on this Platform row.";
            ev.appendChild(p);
          } else {
            evidence.forEach((e) => {
              if (!e.url && e.note) {
                const p = document.createElement("p");
                p.className = "muted";
                p.textContent = (e.label ? e.label + ": " : "") + e.note;
                ev.appendChild(p);
                return;
              }
  ...
  function wireTracker() {
    ...
    if (clear) {
      clear.addEventListener("click", () => {
        state.trackerFilters.platformStatus.clear();
  ...
  function renderFeelGallery() {
    ...
    if (sub) {
      const cat = state.feelCatalog;
      const note =
        cat && !Array.isArray(cat) && typeof cat.galleryNote === "string" && cat.galleryNote.trim()
          ? " " + cat.galleryNote.trim()
          : "";
      sub.textContent =
        items.length +
        " merged Feel fixes · " +
  ...
  function applyHash() {
    ...
    if (raw === "admin-nav" || raw.startsWith("admin-nav/")) {
      showView("admin-nav");
      if (window.HelgoAdminNav && typeof window.HelgoAdminNav.ensure === "function") {
        window.HelgoAdminNav.ensure();
      }
      return;
    }
